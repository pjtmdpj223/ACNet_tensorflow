...
##########################################################
    ...
    print (5)
    tenboard_dir = './tensorboard/cifar-2020-02-05-01-delete/'
    writer = tf.summary.FileWriter(tenboard_dir)
    writer.add_graph(sess.graph)
    #################################################################
#    model_path = './cifar-2020-01-31-11111/cifar10.ckpt-'
    ######################################################
...
#        saver.restore(sess,model_path)
        ...
        for i in range(2001):
            global_step = i
            image_out, label_batch_one_hot_out = sess.run(next_element)
            lr = sess.run(learning_rate)
            _= sess.run(train_step_main, feed_dict={imgs: image_out, label_batch_placeholder: label_batch_one_hot_out,training: True}) 
            ...
            if(i%250 == 0):
                ...
                summary = tf.Summary(value=[
                        tf.Summary.Value(tag="eval_accuracy_main", simple_value= eval_accuracy1),])

                writer.add_summary(summary, i)
                #########
                [train_accuracy1,loss_out1,s] = sess.run([accuracy_main,losses_main,summ], feed_dict={imgs: image_out, label_batch_placeholder: label_batch_one_hot_out,training: False})
                writer.add_summary(s, i)
                print('loss_main:',loss_out1)
                print ("train_accuracy_main:",train_accuracy1)
                print ("eval_accuracy_main:",eval_accuracy1)

                saver.save(sess, './cifar-2020-02-07-01/cifar10.ckpt',global_step=i)    
